...
import com.proxgrind.utils.ContextContentProvider;
...
public class UsbSerialControl implements DriverInterface<String, UsbManager> {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (action != null) {
                    ...
                    if (action.equals(ACTION_PERMISSION)) {
                        ...
                        Log.d(LOG_TAG, "USB Permission result: " + granted);
                        if (granted) {
                            ...
                            if (init1()) {
                                ...
                                //不成则打印到LOG
                                Log.e(LOG_TAG, "NAME_DRIVER_USB_UART: init failed after permission!");
                            }
                        } else {
                            Log.e(LOG_TAG, "NAME_DRIVER_USB_UART: permission denied by user!");
                        }
                    }
    ...
    public void register(DevCallback<String> callback) {
        Log.d(LOG_TAG, "register() called with callback: " + (callback != null ? callback.getClass().getName() : "null"));
        mCallback = callback;
        register1();
    }
    ...
    private boolean init1() {
        ...
        if (!usbManager.hasPermission(usbDevice)) {
            Log.d(LOG_TAG, "Requesting USB permission for device: " + usbDevice.getDeviceName());
            //发送广播申请权限
            PendingIntent intent;
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
                intent = PendingIntent.getBroadcast(mContext, 0, new Intent(ACTION_PERMISSION), PendingIntent.FLAG_IMMUTABLE | PendingIntent.FLAG_UPDATE_CURRENT);
            } else {
                intent = PendingIntent.getBroadcast(mContext, 0, new Intent(ACTION_PERMISSION), PendingIntent.FLAG_UPDATE_CURRENT);
            }
    ...
    private void register1() {
        Log.d(LOG_TAG, "register1() called, isRegister=" + isRegister);
        if (!isRegister) {
            unregister1();
            try {
                // Android 13+ (API 33+) erfordert RECEIVER_EXPORTED oder RECEIVER_NOT_EXPORTED
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.TIRAMISU) {
                    mContext.registerReceiver(usbReceiver, filter, Context.RECEIVER_NOT_EXPORTED);
                } else {
                    mContext.registerReceiver(usbReceiver, filter);
                }
                isRegister = true;
                Log.d(LOG_TAG, "BroadcastReceiver registered successfully");
            } catch (Exception e) {
                Log.e(LOG_TAG, "Failed to register BroadcastReceiver", e);
            }
        }
    }
